...
class DataBaseHandler:
    ...
    def authenticate_user(self, username: str, password: str) -> User:
        ...
        with pool.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(f'SELECT UserID, Username, Email, FirstName, LastName, Password FROM Users WHERE Username = \'{username}\' AND Password = \'{password}\'')
                rows = cur.fetchall()
                if len(rows) == 0:
                    return None
                user = User(rows[0].keys[0], rows[0].keys[1], rows[0].keys[2], rows[0].keys[3], rows[0].keys[4], rows[0].keys[5])
                return user